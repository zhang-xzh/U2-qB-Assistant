...
const sendOk = (sendResponse: (response: any) => void, data: any) => 
    sendResponse({ success: true, data });

const sendError = (sendResponse: (response: any) => void, error: any) =>
    sendResponse({ success: false, error: error instanceof Error ? error.message : String(error) });

const fetchTextOrThrow = async (url: string, options: RequestInit) => {
