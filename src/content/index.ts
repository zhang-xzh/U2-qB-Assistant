...
(async () => {
    ...
    const getStateUI = (state: string): StateUI =>
        STATE_UI[state] || { i: 'fa-question-circle', c: '#bdc3c7', act: 'start', ci: 'fa-play' };

    const normalizeHash = (hash: string | undefined) => String(hash || '').toLowerCase();

    const qbFetch = async (path: string, options: RequestInit = { method: 'GET' }) => {
    ...
    const updateTorrentRowUI = (torrent: any) => {
    ...
    const renderBox = (tid: string, hash: string | null = null) => {
